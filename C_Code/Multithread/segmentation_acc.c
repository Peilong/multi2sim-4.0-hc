...
void seg_write(int step, volatile float* bus, float array[m][p])
{
	int i, j;
	for (i = 0; i < m; i++)
		for (j = 0; j < p; j++)
...
//#pragma AP INTERFACE ap_bus port = C_bus
	...
	//#pragma AP PIPELINE II = 1
		if (i%4 == 0)
		{
            ...
            seg_write(i, C_bus, curvature_motion_part1);
        }
		else if (i%4 == 1)
		{
			...
			fetch(i+1, B_bus, u1);
			compute(phi0, phi1, phi2, u0, curvature_motion_part1, c1, c2);
			seg_write(i, C_bus, curvature_motion_part0);
		}
		else if (i%4 == 2)
		{
	...
	compute(phi2, phi3, phi0, u0, curvature_motion_part1, c1, c2);
	seg_write(58, C_bus, curvature_motion_part0);
	seg_write(59, C_bus, curvature_motion_part1);
}
