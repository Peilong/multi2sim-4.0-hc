...
#define TIMES 8
void des_crypt( unsigned long SK[32],
                unsigned char input[8 * NUM_DATA],
                unsigned char output[8 * NUM_DATA]
                )
{
    unsigned long X, Y, T;
    int iter, times, idx;
    /*timer(0);*/
    for (iter = 0; iter<NUM_DATA; ++iter) {
     
      GET_UINT32_BE( X, input, 0 + iter*8 );
      GET_UINT32_BE( Y, input, 4 + iter*8);
...

	{
      ...
      PUT_UINT32_BE( Y, output, 0 + iter*8 );
      PUT_UINT32_BE( X, output, 4 + iter*8);
    }
    /*timer(1);*/
}
#else
