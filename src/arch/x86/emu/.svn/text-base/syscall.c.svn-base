...
//========= Peilong Code ===========//
#include <x86-timing.h>  //Peilong Code Here
#include <misc.h>        //Peilong Code Here

/* Main Processor Global Variable */
extern struct x86_cpu_t *x86_cpu; //Peilong Code Here
extern struct x86_uop_t *uop; //Peilong Code Here

extern int syscallflag_gettimeofday; //Peilong
extern int syscallflag_acc; //Peilong
...
static int x86_sys_gettimeofday_impl(struct x86_ctx_t *ctx)
{
	...
	//printf ("\nI am calling system call 'gettimeofday'!\n"); //Peilong
	//printf ("Register EAX = %d\n", regs->eax);  //Peilong
	//printf ("Fetch Q:%d\n",uop->in_fetch_queue);	
	//printf ("Fetch Q:%d, UOP Q:%d, Instr Q:%d, Load Q:%d, Store Q:%d, Event Q:%d, ROB:%d, UOP Trace lst:%d\n",\
	//uop->in_fetch_queue, uop->in_uop_queue, uop->in_iq, uop->in_lq, uop-> in_sq, uop->in_event_queue, uop->in_rob, uop->in_uop_trace_list);

	//printf ("Cycle when getting into this call is %lld\n\n", x86_cpu->cycle); //Peilong
	//x86_cpu -> cycle += 100; //Peilong
	...
	//printf ("Cycle when getting out of this call is %lld\n", x86_cpu->cycle); //Peilong
...
static int x86_sys_setitimer_impl(struct x86_ctx_t *ctx)
{
	...
	//printf ("\nI am calling system call 'setitimer'!\n"); //Peilong
	//printf ("Register EAX = %d\n", regs->eax);  //Peilong
	//printf ("Register EBX = %d\n", regs->ebx);  //Peilong
...
static int x86_sys_nanosleep_impl(struct x86_ctx_t *ctx)
{
	...
	//printf ("\nI am calling system call 'nanosleep'!\n"); //Peilong
	//printf ("Register EAX = %d\n", regs->eax);  //Peilong
...
//==================== Peilong acceleration ===================//
/*
 * System call 'acc' (code 330)
 */

static int x86_sys_acc_impl(struct x86_ctx_t *ctx)
{
	/* Run acc call */
	//printf("system call 'acc' is called!\n\n");
	syscallflag_acc = 1;	
	return x86_acc_call(ctx);
}
