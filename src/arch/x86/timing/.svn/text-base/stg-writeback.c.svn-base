...
/*
		if (uop->when <= x86_cpu->cycle) //Peilong
		{
			struct x86_regs_t *regs = uop->ctx->regs; //Peilong
			//if (uop->uinst->opcode == 58) //Peilong
			//{
				printf ("Moniter Writeback stage: \n\tRegister EAX content:  %lld", regs -> eax);	//Peilong
				printf ("\tcpu cycle %lld\t,Uop when %lld\n", x86_cpu->cycle, uop->when); //Peilong		
			//}
			if (regs -> eax == 78) //Peilong
				printf ("\tInstr queue is inserted with 'gettimeofday' syscall!\
				\tcpu cycle %lld\n",x86_cpu->cycle); //Peilong
		}
*/
		else if (uop->when > x86_cpu->cycle)
		{	
			break;
		}
