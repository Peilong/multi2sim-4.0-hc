...
/*		
	//======= Peilong ========//
	struct x86_regs_t *regs = uop->ctx->regs; //Peilong

	if (uop->uinst->opcode == 58) //Peilong
	{
		printf ("Moniter EQ insert: \n\tRegister EAX content:  %d", regs -> eax);	//Peilong
		printf ("\tcpu cycle %lld\n", x86_cpu->cycle); //Peilong
		//printf ("Register EBX content: %d\n", regs -> ebx);	//Peilong
		//printf ("Register ECX content: %d\n", regs -> ecx);	//Peilong		
		//printf ("Register EDX content: %d\n\n", regs -> edx);	//Peilong		
	}	
	if (regs -> eax == 162) //Peilong
		printf ("\tEvent queue is inserted with 'nanosleep' syscall!\tcpu cycle %lld\tuop opcode: %d\n",x86_cpu->cycle, uop->uinst->opcode); //Peilong
	if (regs -> eax == 78) //Peilong
		printf ("\tEvent queue is inserted with 'gettimeofday' syscall!\tcpu cycle %lld\n",x86_cpu->cycle); //Peilong
*/
...
/*
	...
	struct x86_regs_t *regs = uop->ctx->regs; //Peilong
	if (regs -> eax == 162) //Peilong
		printf ("\nEQ extract: \n\tEvent queue is inserted with 'nanosleep' syscall!\tcpu cycle %lld\n",x86_cpu->cycle); //Peilong
*/
	...
	//printf ("\nEvent Queue Extract: UOP = %d\n\tcpu cycle %lld",uop->uinst->opcode,x86_cpu->cycle);	//Peilong
