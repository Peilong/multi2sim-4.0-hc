...
/********************************************************************/
/* Added Code:
/* Purpose:	Dump the detailed access number in data cache
/*			Calculate temperal locality and spatial locality
/********************************************************************/
		int count_way;
		int count_set;
		int count_offset;
		unsigned long long cache_access = 0;  //Number of cache accesses
		unsigned long long hit_cache_set = 0; //Include different ways
		unsigned long long hit_cache_byte = 0;
		unsigned long long rd_allcells = 0;   //Summation of reuse distance for all cells
		
		//A counter to sum up the access number of each single cache line.
		unsigned long long cache_single_set_count = 0;
		
		if (strcmp(mod->name,"mod-dl1-0")==0) //Peilong added
		{
			for(count_set=0; count_set<SET; count_set++)
			{
				//counter for a single cache line accumulated access #. cleared as 0 for each set loop.
				cache_single_set_count = 0;
				fprintf(f, "-Set[%d]: \n", count_set);
				for(count_offset=0; count_offset<OFFSET; count_offset++)
				{
					for(count_way=0; count_way<WAY; count_way++)
					{
						fprintf (f, "Way%d:", count_way);
						rd_allcells += dcache[count_way][count_set][count_offset].rd;
						cache_access += dcache[count_way][count_set][count_offset].access;
						cache_single_set_count  += 
							dcache[count_way][count_set][count_offset].access;
						
						if (dcache[count_way][count_set][count_offset].access > 0)
							hit_cache_byte ++;
						fprintf(f, "[%8llu]  ", 
							dcache[count_way][count_set][count_offset].access);
					}
			...
			fprintf(f, "Number of hitted cache line set : %llu\n", hit_cache_set);
			fprintf(f, "Number of hitted cache line byte: %llu\n", hit_cache_byte);
			fprintf(f, "Spatial Locality: %f\n", hit_cache_byte/(1.0*hit_cache_set*OFFSET*WAY));
			fprintf(f, "Temperal Locality: %f\n", rd_allcells /(1.0*WAY*SET*OFFSET));
			fprintf(f, "Total cache accesses: %llu\n\n", cache_access);
		}
