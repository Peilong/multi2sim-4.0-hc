...
#ifdef ACC_CYCLE
#define ACC_CYCLE_MIN 3319838    //Peilong
#define ACC_CYCLE_MAX 11005755   //Peilong
#endif
...
void mod_handler_nmoesi_load(int event, void *data)
{
	...
	if (event == EV_MOD_NMOESI_LOAD_ACTION)
	{
		...
		#ifdef ACC_CYCLE
		if (esim_cycle >= ACC_CYCLE_MIN && esim_cycle <= ACC_CYCLE_MAX)//Peilong added
		#endif
		#if 0
			if (strcmp(mod->name,"mod-dl1-0")==0) //Peilong added
			{
				struct cache_t *cache = mod->cache; //Peilong Added
				int way = stack->way;
				int offset = stack->addr & cache->block_mask;
				int set = stack->set;
				dcache[way][set][offset].access ++;
				dcache[way][set][offset].rd_sum += esim_cycle - dcache[way][set][offset].cycle;
				dcache[way][set][offset].cycle = esim_cycle;
				dcache[way][set][offset].rd = 
					(dcache[way][set][offset].access <= 1)?
						(0):
						(dcache[way][set][offset].rd_sum / (dcache[way][set][offset].access -1));
				//printf("%lld stack_addr:%u stack_tag:%d stack_set:%d stack_way:%d set:%d offset:%d\n",
				//	esim_cycle, stack->addr, 
				//	stack->tag, stack->set, stack->way, dcache_set, dcache_offset);
...
void mod_handler_nmoesi_store(int event, void *data)
{
	...
	if (event == EV_MOD_NMOESI_STORE_ACTION)
	{
		...
		#ifdef ACC_CYCLE
		if (esim_cycle >= ACC_CYCLE_MIN && esim_cycle <= ACC_CYCLE_MAX)
		#endif
		#if 0
			if (strcmp(mod->name,"mod-dl1-0")==0) //Peilong added
			{
